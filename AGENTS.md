...
- **Config (`spur.conf`, `config.rs`)** — removing/renaming a field or changing its type breaks deployed configs. New fields need `#[serde(default)]`.

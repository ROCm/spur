...
.. list-table::
   ...
   * - ``scontrol show assoc_mgr``
     - Reports per-user usage against QOS and association caps, plus the caps a record is already over. Filters to one user (``users=<name>`` or a bare name); Slurm's ``accounts=``, ``qos=`` and ``flags=`` selectors are not accepted, and scopes with no jobs are omitted rather than listed as empty records. Fairshare and priority internals Slurm prints (``SharesRaw``, ``UsageRaw``, ``Lft-Rgt``) are not included.
   * - QOS ``GrpWall``

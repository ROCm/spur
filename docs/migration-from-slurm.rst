...
.. list-table::
   ...
   * - ``scontrol show assoc_mgr``
     - Caps print as ``Limit(Consumed)`` with ``N`` for no limit, as in Slurm, and an ``OverLimit`` field names caps already exceeded. Filters to one user (``users=<name>`` or a bare name); Slurm's ``accounts=``, ``qos=`` and ``flags=`` selectors are not accepted. Users are listed on their own lines under a scope rather than nested in a ``User Limits=`` field, and an account's group figures are stated once for the account instead of repeated per association row. Fairshare and priority internals Slurm prints (``SharesRaw``, ``UsageRaw``, ``Lft-Rgt``, ``GrpTRESMins``) are not included.
   * - QOS ``GrpWall``

...
.. list-table::
   ...
   * - ``preempt_mode``
     ...
     - What the scheduler does to a running job in this partition when an
       ...
       action. ``"off"`` — an explicit no-action; for this field the two
       resolve identically, differing only in that ``scontrol show partition``
       reports ``PreemptMode=NONE`` versus ``PreemptMode=OFF``, so you can
       tell whether anyone configured it.

       A running job's QOS ``preemptmode`` always overrides this field,
       including ``off`` (see ``preemptmode`` in :doc:`accounting`), so a
       QOS can shield its jobs from preemption regardless of what this field
       says. Set the field back to unset with ``PreemptMode=NONE`` (or an
       empty value) via ``scontrol update``.

       When a job spans several partitions, the most aggressive
       ``preempt_mode`` among them wins (``cancel`` > ``requeue`` >
       ``suspend`` > ``off``). Whether preemption is attempted at all is
       governed by ``scheduler.preempt_type``, not by this field. See
       :doc:`accounting` for the complete decision table.
       ...
       .. important::
          ...
          for every job running inside its quota, unset/``off`` on this field
          still means no preemption from the partition side — though a QOS can
          still shield or preempt independently of it.
   * - ``preempt_exempt_time``
     - integer or null
     - ``null`` (inherit global)

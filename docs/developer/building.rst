...
The mesh, pod, and service CIDRs are pinned to test-only ranges
(``10.46.0.0/16`` / ``10.47.0.0/16`` / ``10.48.0.0/16``) rather than the product
defaults, because the product's default WireGuard CIDR (``10.44.0.0/16``)
overlaps the CI runners' underlay network — a mesh whose ``/16`` covers the
nodes' real addresses routes the underlay into ``spur0`` and stalls k0s/calico.
...
``wireguard-tools`` (``wg``/``wg-quick``) and a data plane — the ``wireguard``
kernel module or the ``wireguard-go`` userspace binary. Nodes lacking these are
skipped, not failed.

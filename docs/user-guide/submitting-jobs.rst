...
An array holds at most 100,000 tasks. Ranges may overlap and are deduplicated
before that limit applies, so ``--array=0-99999,0-99999`` is accepted as 100,000
distinct tasks. A spec that names more than 200,000 indices before
deduplication is rejected however far they would collapse, which keeps the cost
of parsing a spec bounded.

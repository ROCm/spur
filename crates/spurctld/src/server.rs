...
fn proto_to_job_spec(spec: JobSpec) -> Result<spur_core::job::JobSpec, Status> {
    ...
    // Unchecked, an unexpandable pattern falls back to a literal name matching
    // no node: a bad --nodelist then never schedules, and a bad --exclude
    // silently excludes nothing. Counting rather than expanding avoids building
    // a list the scheduler rebuilds later anyway.
    for (flag, pattern) in [("nodelist", &spec.nodelist), ("exclude", &spec.exclude)] {
        if !pattern.is_empty() {
            spur_core::hostlist::count(pattern).map_err(|e| {

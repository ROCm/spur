...
/// short-circuits on `preempt_mode = Off` and `qos_preempt_allowed` gates the
/// victim. Reclaim consults neither, so reusing that selection would let a job
/// that can never be placed — `--gres=gpu:8` where the largest node has 4 —
/// requeue one borrowed job per cycle forever (D2, §8.1).
///

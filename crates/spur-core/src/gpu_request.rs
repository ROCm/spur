...
pub enum GpuRequestError {
    ...
    Conflict,
    #[error("conflicting GPU types in gres entries: expected one type, found multiple")]
    ConflictingGresTypes,
    #[error("gpus total ({total}) is less than the number of nodes ({nodes}); each node needs at least one GPU")]
    TotalLessThanNodes { total: u32, nodes: u32 },
    #[error("gpus_per_task requires at least one task")]
    PerTaskNeedsTasks,
            ...
            if name == "gpu" {
                ...
                if let Some(t) = gtype {
                    if let Some(ref existing) = gpu_type {
                        if existing != &t {
                            return Err(GpuRequestError::ConflictingGresTypes);
                        }
                    } else {
                        gpu_type = Some(t);
                    }
...
mod tests {
    ...
    fn resolve_rejects_mixed_gres_gpu_types() {
        ...
        assert_eq!(
            resolve_gpu_demand(&spec),
            Err(GpuRequestError::ConflictingGresTypes)
        );
    }
    ...
    #[test]
    fn parse_flag_strips_gpu_prefix() {
        let r = GpuRequest::parse_flag("gpu:4").unwrap().unwrap();
        assert_eq!(r, GpuRequest::new(4, None));

        let r = GpuRequest::parse_flag("gpu:mi300x:4").unwrap().unwrap();
        assert_eq!(r, GpuRequest::new(4, Some("mi300x".into())));

        assert_eq!(GpuRequest::parse_flag("gpu:").unwrap(), None);
        assert_eq!(GpuRequest::parse_flag("GPU:").unwrap(), None);
    }

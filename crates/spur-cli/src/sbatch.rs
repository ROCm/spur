...
pub async fn main_with_args(cli_args: Vec<String>) -> Result<()> {
    ...
    let mut args = SbatchArgs::try_parse_from(&merged_args)?;
...
"#;
    ...
    fn test_wrap_command_body_matches_slurm() {
        // Byte-for-byte match with Slurm's src/sbatch/sbatch.c _script_wrap.
        assert_eq!(
    ...
    fn test_choose_body_source_wrap_wins() {
        // --wrap takes precedence even when stdin is a tty.
        assert_eq!(
    ...
    fn test_wrap_string_not_parsed_for_directives() {
        // A wrap command that looks like a directive must not be scanned.
        let directives = parse_sbatch_directives("#SBATCH --nodes=9\n");
        assert_eq!(directives, vec!["--nodes=9"]);
        // The wrap value is never passed through parse_sbatch_directives; the
        // pre-parse only reads a script file. Sanity-check the body is verbatim.
        assert_eq!(

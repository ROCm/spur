        ...
        loop {
            ...
            match action {
                ...
                reporter::RefreshAction::ReportLost => {
                    ...
                    // Re-register the shrunk inventory so the controller's
                    // advertised GRES converges. The NodeUpdate handler only
                    // refreshes total_resources; it does NOT drain or requeue
                    // the job still holding the vanished device.
                    warn!(
                        ?lost,
                        "allocated device vanished; re-registering shrunk inventory (job not drained)"
                    );
                    let commit = converge(&reporter, &fresh, &delta, &mut last_delta).await;
                    publish_registry(&registry, candidate, commit).await;
                }
